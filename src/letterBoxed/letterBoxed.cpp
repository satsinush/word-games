...
namespace LetterBoxed
{
        ...
        std::vector<int> &allPathIndices)
    {
        ...
        if (depth == wordObj.wordString.length())
        {
            ...
            results.push_back({offset, static_cast<int>(currentPathGlobalIndexes.size()), config.letterToSideMapping[currentPathGlobalIndexes.back()], wordObj.score});
            return;
        }

        char targetChar = wordObj.wordString[depth];
        for (unsigned int globalIdx = 0; globalIdx < config.allLetters.size(); ++globalIdx)
        {
        ...
        const std::vector<int> &allPathIndices)
    {
        ...
        {
            double scoreMin = currentWordChain.empty() ? 0.0 : currentWordChain[0]->score;
