...
void LetterBoxedGame::drawPuzzle(const std::array<char, 12> &letters) {
  ...
  std::cout << std::endl;
  std::cout << "      " << up(letters[0]) << " " << up(letters[1]) << " "
            << up(letters[2]) << std::endl;
  std::cout << "    +-------+" << std::endl;
  std::cout << "  " << up(letters[11]) << " |       | " << up(letters[3])
            << std::endl;
  std::cout << "  " << up(letters[10]) << " |       | " << up(letters[4])
            << std::endl;
  std::cout << "  " << up(letters[9]) << " |       | " << up(letters[5])
            << std::endl;
  std::cout << "    +-------+" << std::endl;
  std::cout << "      " << up(letters[8]) << " " << up(letters[7]) << " "
            << up(letters[6]) << std::endl
            << std::endl;
}
...
LetterBoxed::Config LetterBoxedGame::getConfigFromUser() {
  ...
  if (preset == 1) {
    ...
    config.maxDepth =
        Utils::Input::promptInt("Max words per solution", 2, 1, 4);
    config.minWordLength = Utils::Input::promptInt("Min word length", 3, 1);
    config.minUniqueLetters =
        Utils::Input::promptInt("Min unique letters per word", 2, 1);
...
LetterBoxed::Config LetterBoxedGame::getConfigFromArgs(
  ...
  if (letters.empty() || letters.size() != 12) {
    throw std::invalid_argument(
        "Invalid letters argument. Must provide exactly 12 letters.");
  }
...
void LetterBoxedGame::printSolutions(
    const std::vector<LetterBoxed::Solution> &solutions, const int limit) {
  int lastNumWords = 0;
