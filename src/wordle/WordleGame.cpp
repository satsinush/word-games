...
namespace Game
{
    ...
    Wordle::Config WordleGame::getConfigFromUser()
    {
        ...
        config.maxDepth = Utils::Input::promptInt("Search depth for entropy calculation (0-2)", 0, 0, 2);
        config.excludeUncommonWords = Utils::Input::promptBool("Exclude uncommon words from suggestions?", false);

        return config;
    }

    Wordle::Config WordleGame::getConfigFromArgs(const std::map<std::string, std::string> &args)
    {
        Wordle::Config config;
        config.maxDepth = Utils::Input::getArgValue(args, "maxDepth", 0);
        config.excludeUncommonWords = Utils::Input::getArgValue(args, "excludeUncommonWords", false);
        return config;
    }
    ...
    std::vector<Wordle::Feedback> WordleGame::getFeedbackFromArgs(const std::map<std::string, std::string> &args)
    {
        std::vector<Wordle::Feedback> feedbackHistory;
    ...
    void WordleGame::printResults(const Wordle::Result &result)
    {
        ...
        if (!result.sortedGuesses.empty())
        {
            ...
            std::cout << std::setw(12) << "Word" << std::setw(12) << "Entropy" << std::setw(15) << "Probability" << "\n";
    ...
    void WordleGame::saveResults(const Wordle::Result &result, const std::string &possibleFile, const std::string &guessesFile)
    {
        // Save possible words
        ...
        if (guessesOut.is_open())
        {
            ...
            for (const auto &guess : result.sortedGuesses)
            {
                guessesOut << guess.word.wordString << ","
                           << guess.entropy << ","
                           << guess.probability << "\n";
            }
            guessesOut.close();
        ...
        while (true)
        {
            ...
            while (true)
            {
                ...
                if (input == "solve")
                {
                    ...
                    Wordle::Result result = Wordle::runWordleSolver(wordVec, feedbackHistory, config);
