...
protected:
  ...
  virtual TGuessType
  createGuess(const TCandidateType &candidate, double entropy,
              double probability,
              const std::vector<double> &entropyList) const = 0;
...
public:
  ...
  TResultType solve(const std::vector<TCandidateType> &allCandidates,
    ...
    // If maxDepth is 0, skip entropy calculation and just return filtered
    // candidates
    ...
    for (const auto &guessCandidate : allCandidates) {
      ...
      std::vector<double> entropyList = calculateEntropyAtDepths(
          guessCandidate, possibleCandidates, config.maxDepth);
      double entropy = entropyList.empty() ? 0.0 : entropyList.back();

      TGuessType guess =
          createGuess(guessCandidate, entropy, probability, entropyList);
      guesses.push_back(guess);
    }
...
private:
   ...
   * This is much more efficient than calling calculateEntropyAtDepth multiple
   * times
   */
  std::vector<double> calculateEntropyAtDepths(
      const TCandidateType &guessCandidate,
    ...
    for (uint8_t depth = 2; depth <= maxDepth; ++depth) {
      ...
      for (const auto &group : feedbackGroups) {
        if (group.second.size() >
            1) // Only calculate if there are multiple candidates
        {
          double prob = static_cast<double>(group.second.size()) / total;
          ...
          for (const auto &nextGuess : possibleCandidates) {
            ...
            std::vector<double> subEntropyList =
                calculateEntropyAtDepths(nextGuess, group.second, depth - 1);
