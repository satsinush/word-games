...
public:
  ...
  const std::vector<FunctionProfile *> &getChildren() const {
    return childList;
  }
...
private:
  ...
  void logProfile(const FunctionProfile &profile, const int64_t totalTime,
                  const int depth = 0, const bool corner = false);
...
/*
 * RAII class to automatically profile a scope (function/block) using the
 * Profiler.
 */
class ProfileScope {
public:
